...
public class hardware_question5 extends AppCompatActivity implements View.OnClickListener {
    TextView level;
    TextView question;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        questionC.setTheQuestion("Which of the are normally used initialize a computer system's hardware ?");
        questionC.setAnswer1("Volatile memory");
        questionC.setAnswer2("External mass memory");
        questionC.setAnswer3("Static memory");
        questionC.setRightAnswer("Bootstrap memory");
        ...
        questionBar.setText("");
        level.setText("Hardware Question 5");
    ...
    public void onClick(View v) {
        if (v.getId() == R.id.choice1){
            if (choice1.getText() == questionC.getRightAnswer()){
                reference.child(mAuth.getCurrentUser().getUid()).child("HardwareAchievement5").setValue(true);
              openWinDialog();
            }
            else
            openLoseDialog();
    ...
    private void openWinDialog() {
        dialog.setContentView(R.layout.hardware_ach_1);
        dialog.getWindow().setBackgroundDrawable(new ColorDrawable(Color.TRANSPARENT));
