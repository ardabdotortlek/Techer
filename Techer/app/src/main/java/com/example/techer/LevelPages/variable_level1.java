...
public class variable_level1 extends AppCompatActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        questionC.setTheQuestion("Which of the following rule is wrong when naming java variables?");
        questionC.setAnswer1("Having underscore in the word");
        questionC.setAnswer2("Having digits in the word");
        questionC.setAnswer3("Writing the word without spaces");
        questionC.setRightAnswer("Starting naming with a capital letter");
    ...
    public void onClick(View v) {
        if (v.getId() == R.id.choice1){
            if (choice1.getText() == questionC.getRightAnswer()) {
                reference.child(mAuth.getCurrentUser().getUid()).child("1rowSoftwareQuestions").child("question1").setValue(true);
                openWinDialog();
    ...
    private void openWinDialog() {
        ...
        btnOk.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                startActivity(new Intent(getApplicationContext(), variable_level2.class));
                dialog.dismiss();
            }
        });
