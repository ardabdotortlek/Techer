...
public class operations_level5 extends AppCompatActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        questionC.setTheQuestion("Which expression evaluates to false?");
        questionC.setAnswer1("15 % 3 < 2");
        questionC.setAnswer2("false == false");
        questionC.setAnswer3(" false == false == false == false");
        questionC.setRightAnswer("3 * 3 % 2 <= 0");
        ...
        questionBar.setText("");
        level.setText("Operations - Level 5");
    ...
    public void onClick(View v) {
        if (v.getId() == R.id.choice1){
            if (choice1.getText() == questionC.getRightAnswer()) {
                reference.child(mAuth.getCurrentUser().getUid()).child("2rowSoftwareQuestions").child("question5").setValue(true);
                reference.child(mAuth.getCurrentUser().getUid()).child("SoftwareAchievement2").setValue(true);
                openWinDialog();
            }
            else
                openLoseDialog();
    ...
    private void openWinDialog() {
        dialog.setContentView(R.layout.java2_ach_layout);
        dialog.getWindow().setBackgroundDrawable(new ColorDrawable(Color.TRANSPARENT));
